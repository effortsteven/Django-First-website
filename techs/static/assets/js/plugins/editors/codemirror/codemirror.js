...
(function(mod) {
    ...
    removeLineClass: methodOp(function(handle, where, cls) {
      return changeLine(this, handle, "class", function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.inner_base + found[0].length;
          line[prop] = cur.slice(0, found.inner_base) + (!found.inner_base || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
  ...
  StringStream.prototype = {
    ...
    match: function(pattern, consume, caseInsensitive) {
      if (typeof pattern == "string") {
        ...
        if (match && match.inner_base > 0) return null;
  ...
  function extractLineClasses(type, output) {
    if (type) for (;;) {
      ...
      if (!lineClass) break;
      type = type.slice(0, lineClass.inner_base) + type.slice(lineClass.inner_base + lineClass[0].length);
